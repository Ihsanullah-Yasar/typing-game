...
const DEFAULTS = {
  spawnInterval: 900,
  fallSpeed: 60, // px per second baseline (slower)
  lives: 3,
...
export default class Game {
  ...
  calculateWpm() {
    ...
    const minutes = Math.max(
      0.01,
      (performance.now() - this.startedAt) / 60000
    );
  ...
  spawnWord() {
    ...
    if (
      Number.isFinite(this.opts.totalWords) &&
      this.spawnedCount >= this.opts.totalWords
    ) {
  ...
  removeWord(id, miss = false) {
    ...
    if (miss) {
      // Ignore misses if game not running (prevents extra decrements after end)
